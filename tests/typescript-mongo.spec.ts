...
describe('TypeScript Mongo', () => {
  ...
  const schema = buildSchema(/* GraphQL */ `
    ...
    type User @graphqlator(additionalFields: [{ path: "nonSchemaField", type: "string" }]) {
      ...
      linkWithoutDirective: LinkType @computed
      ...
      requiredField: String! @computed
      arrayColumn: [Int]
      arrayColumnMap: [Int] @column @map(path: "myInnerArray")
      basicEmbedded: EmbeddedType! 
      arrayEmbedded: [EmbeddedType!]! 
      nullableEmbedded: [EmbeddedType]
      mappedEmbedded: EmbeddedType @map(path: "innerEmbedded.moreLevel")
      ...
      nullableColumnMapPath: String @map(path: "nullableColumnMap.level")
  ...
  describe('Output', () => {
    ...
    it('Should include only the relevant types', async () => {
      ...
      expect(result).not.toContain('export type MutationDbObject = {');
      expect(result).not.toContain('export type SubscriptionDbObject = {');
    });
